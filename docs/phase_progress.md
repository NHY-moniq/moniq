...
- [ ] "소프트 요약" 탭: 기피패턴 위반 → 패턴별 그룹 + 멤버·날짜 목록 나오는지
- [ ] "소프트 요약" 탭: 미반영 원티드 → 멤버·날짜 목록 나오는지
- [ ] "소프트 요약" 탭: 커스텀 룰 위반 → 항목별 날짜 포함해 나오는지
- [ ] 위반 배너 항상 표시 (하드/커스텀/기피패턴/원티드 칩 형태)
- [ ] 커스텀 룰 위반 메시지에 구체적 날짜 포함 확인 (05-03, 05-10 형식)
...
- [ ] 스케줄 생성 규칙 화면 → "커스텀 규칙" 카드 → 화면 진입 확인
