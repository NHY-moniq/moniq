      ...
      body: stateAsync.when(
        ...
        data: (state) {
          if (state.activeRequest != null) {
            return _ActiveRequestView(
              teamId: teamId,
              teamName: teamName,
              state: state,
            );
          }
          return _CreateRequestView(
            teamId: teamId,
            teamName: teamName,
          );
        },
...
/// 활성 수집 요청 현황
class _ActiveRequestView extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // 팀원별 엔트리 그루핑
    final groupedByUser = <String, _UserEntryGroup>{};
    for (final ew in state.allEntries) {
      final uid = ew.entry.userId;
      groupedByUser.putIfAbsent(
        uid,
        () => _UserEntryGroup(displayName: ew.displayName, dates: []),
      );
...
class _UserEntryGroup {
  _UserEntryGroup({required this.displayName, required this.dates});
